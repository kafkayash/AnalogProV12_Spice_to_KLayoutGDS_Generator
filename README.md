...
## How I created this showcase branch

I keep this as a separate branch so the main README does not become too heavy.
...
Create and switch to the showcase branch:

```bash
git checkout main
git pull origin main
git checkout -b showcase-screenshots
```

Place this README as the branch README:

```bash
cp ~/Downloads/README_showcase_branch.md README.md
```

Make sure the assets are present:

```bash
ls docs_assets/screenshots
ls docs_assets/terminal_logs
```

Do not add generated run folders:
...
If `skyspice_runs/` appears, do not commit it. Keep only selected screenshots and logs.

Commit the showcase branch:

```bash
git add README.md docs_assets
git commit -m "Add visual showcase branch"
git push -u origin showcase-screenshots
```

After pushing, open the branch on GitHub:

```text
https://github.com/kafyash/AnalogProV12_Spice_to_KLayoutGDS_Generator/tree/showcase-screenshots
```

## Optional link to keep in main README

If I want the clean main README to point to this detailed visual branch without adding all screenshots to main, I can add one small line in the main README:

```md
For the detailed visual showcase with KLayout screenshots and terminal logs, see the [`showcase-screenshots`](https://github.com/kafyash/AnalogProV12_Spice_to_KLayoutGDS_Generator/tree/showcase-screenshots) branch.
```

That keeps the main branch clean and lets this branch act like a visual lab notebook.
